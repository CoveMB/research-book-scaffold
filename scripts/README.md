...
Use `bash scripts/update-skills-vendors.sh` when the vendored skill repositories have new upstream commits. It fetches repository refs, fast-forwards the configured vendor submodules, refreshes local skill wrappers and install reports, and runs the external-skill and doctor checks.
