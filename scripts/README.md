...
Use `make install-subagent-orchestrator` only when the optional subagent plugin should install repo-local skills. That target passes `--scope project` to the vendored installer and does not activate the prompt gate or install project agents.
